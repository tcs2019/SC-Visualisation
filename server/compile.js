...
const Compile = {
  ...
  findImports(importFile) {
    ...
    try {
      ...
      const result = fs.readFileSync(config.ETH_ContractFolder + importFile, 'utf8');
  ...
  buildContract(contract) {
    const contractFile = config.ETH_ContractFolder + contract;
    const jsonOutputName = `${path.parse(contract).name}.json`;
    const jsonOutputFile = config.ETH_JSON_Compile_Folder + jsonOutputName;
    ...
    const output = solc.compileStandardWrapper(solcInputString, this.findImports);
    ...
    const buildDir = config.ETH_JSON_Compiled_Folder;
    if (!fs.existsSync(buildDir)) {
      fs.mkdirSync(buildDir);
    }
    // Write the output JSON
    fs.writeFileSync(config.ETH_JSON_Compiled_Folder + jsonOutputName, formattedJson);
